#!/usr/bin/env bash
# Runs the TFLite benchmark_model binary on an Android device's GPU.
# Usage: ./benchmark_gpu.sh <model.tflite> [device_name_or_id]
...
# "gpu") as the outer layer under benchmark/, with gpu/ as the inner layer:
# tflite_models/original/foo.tflite's benchmark lands in
# benchmark/original/gpu/. Models outside tflite_models/ land flat in
# benchmark/gpu/.
...
OUT_DIR="$SCRIPT_DIR${REL_SUBDIR:+/$REL_SUBDIR}/gpu"
