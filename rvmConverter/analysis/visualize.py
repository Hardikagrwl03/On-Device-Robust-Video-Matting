...
# give it a generous budget rather than risk a false "stuck" timeout.
...
# activeTarget can be truthy and nodeCount already near its final value
# while #canvas's getBBox() is still a tiny placeholder like 100x21 -- one
# poll interval later it snaps to the graph's real size and stays there.
# Triggering export() during that window makes netron measure the same
# tiny bbox and silently produce a near-empty image instead of an error,
# so readiness must be judged from the bbox itself, not from
# node/activeTarget alone.
_STATE_SCRIPT = """
...
def parse_args():
    ...
    parser.add_argument(
        "--format", choices=("svg", "png"), default=DEFAULT_FORMAT, help=f"Output image format (default: {DEFAULT_FORMAT})."
    )
    parser.add_argument(
        "--output",
        default=None,
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="rvm-visualize-") as download_dir:
        try:
