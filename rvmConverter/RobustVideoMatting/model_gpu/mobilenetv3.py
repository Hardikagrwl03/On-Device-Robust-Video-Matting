...
from .lraspp import GlobalAvgPool2d
...
class MobileNetV3LargeEncoder(MobileNetV3):
    def __init__(self, pretrained: bool = False):
        ...
        # Several InvertedResidual blocks (the ones with use_se=True above)
        # contain a torchvision SqueezeExcitation submodule, which pools via
        # its own nn.AdaptiveAvgPool2d(1) -- same GATHER_ND-on-export issue
        # as lraspp.py's aspp2 pool, fixed the same way: swap it for a plain
        # spatial mean, which is exact for this global (output size 1) case
        # and lowers to a GPU-delegate-supported MEAN reduction instead.
        for module in self.modules():
            if isinstance(module, SqueezeExcitation):
                module.avgpool = GlobalAvgPool2d()
