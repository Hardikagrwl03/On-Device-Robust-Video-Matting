...
class AvgPool(nn.Module):
    def __init__(self):
        super().__init__()
        # ceil_mode=True + count_include_pad=False make the last window's
        # normalization depend on how many elements it actually contains
        # (not a fixed kernel area), which torch.export can't lower to a
        # plain pooling op -- it falls back to a generic
        # STABLEHLO_REDUCE_WINDOW, which the TFLite GPU delegate doesn't
        # support. Dropping both (kernel=stride=2, no padding) is exactly
        # equivalent whenever each pooled dimension is even at every stage
        # (i.e. the traced height/width are multiples of 8) -- true for the
        # even camera resolutions this export targets -- and lowers to a
        # plain, GPU-delegate-supported AVERAGE_POOL_2D instead.
