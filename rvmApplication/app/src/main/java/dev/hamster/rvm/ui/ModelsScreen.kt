 ...
 * 104 MB transfer the moment the user navigated back to the home screen.
 */
...
fun ModelsScreen(onNavigateBack: () -> Unit) {
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            items(gpuSpecs, key = { it.fileName }) { spec ->
                ModelCard(
                    spec = spec,
                    state = states[spec.fileName] ?: ModelDownloadState.NotInstalled,
            ...
            items(originalSpecs, key = { it.fileName }) { spec ->
                ModelCard(
                    spec = spec,
                    state = states[spec.fileName] ?: ModelDownloadState.NotInstalled,
                    onDownload = { repository.download(spec) },
...
private fun ModelCard(
    ...
    val title = stringResource(
        when (spec.backbone) {
            "mobilenetv3" -> R.string.model_backbone_mobilenetv3
            else -> R.string.model_backbone_resnet50
        }
    )
    val downsample = if (spec.downsampleTag == "auto") {
        stringResource(R.string.downsample_auto_label)
    } else {
        stringResource(R.string.downsample_percent_format, spec.downsampleTag.toInt())
    }
    ...
    val detail = when (state) {
        ...
        else -> Formatter.formatShortFileSize(context, spec.sizeBytes)
    }
